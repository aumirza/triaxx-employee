...
const FloorPersonsModal: React.FC<FloorPersonsModalProps> = ({ open, onClose, onSelect }) => {
  const [persons, setPersons] = useState(1);
  const [selectedFloor, setSelectedFloor] = useState<string>('F-01');

  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center transition-opacity duration-300 ease-in-out ${open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
    >
      <div className="absolute inset-0 bg-[rgba(0,0,0,0.15)] backdrop-blur-sm" onClick={onClose} />
      <div className="bg-white rounded-2xl w-[340px] max-w-full shadow-xl overflow-hidden relative z-10">
        ...
        <div className="bg-gradient-to-r from-[#6A1B9A] to-[#D32F2F] p-4 text-white rounded-t-2xl">
          <div className="text-xl font-bold">Floor Map</div>
          <div className="text-sm font-normal">Select Floor and Persons</div>
        </div>
        {/* Content */}
        <div className="p-6">
          <div className="mb-6">
            <div className="font-semibold mb-2">Select Persons:</div>
          ...
          <div className="mb-6">
            <div className="font-semibold mb-2">Select Floor:</div>
            <div className="grid grid-cols-3 gap-3">
              {floors.map((floor) => (
                <button
                  key={floor}
                  className={`py-3 rounded-lg text-base font-medium shadow-sm transition-all ${selectedFloor === floor ? 'bg-gradient-to-r from-[#6A1B9A] to-[#D32F2F] text-white' : 'bg-[#f7f2fa] text-[#6A1B9A]'}`}
