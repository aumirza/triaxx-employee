...
const LogoutModal: React.FC<LogoutModalProps> = ({ isOpen, onClose, onConfirm }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-tranparent bg-opacity-50" onClick={onClose}>
      <div 
        className="relative w-full max-w-xl p-16 bg-white rounded-3xl shadow-lg flex flex-col items-center justify-center"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-3xl font-bold mb-8 text-gray-800 text-center">Would you like to Sign out?</h2>
        
        <div className="flex items-center justify-center mb-10">
          <div 
            className={`w-6 h-6 mr-4 rounded-md flex items-center justify-center cursor-pointer border-2 ${savePassword ? 'bg-green-500 border-green-500' : 'border-gray-300'}`}
