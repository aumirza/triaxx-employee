...
const navItems = [
  { name: 'Orders', path: '/orders', icon: QuickOrderIcon },
  { name: 'Table', path: '/table', icon: TableIcon },
  { name: 'Chats', path: '/team-chats', icon: ChatsIcon },
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-14 py-1.5 bg-white shadow-[0_-2px_4px_rgba(0,0,0,0.05)] lg:hidden flex justify-around items-center px-4 z-40">
      {navItems.map((item) => {
        ...
        return (
          <Link to={item.path} key={item.name} className="flex flex-col items-center justify-center w-1/4">
            <img src={item.icon} alt={item.name} className={`w-6 h-6 mb-1 ${isActive ? 'text-transparent bg-clip-text bg-gradient-to-r from-[#6A1B9A] to-[#D32F2F]' : 'text-gray-500'}`} />
            <span className={`text-xs font-medium ${isActive ? 'text-transparent bg-clip-text bg-gradient-to-r from-[#6A1B9A] to-[#D32F2F]' : 'text-gray-500'}`}>
              {item.name}
            </span>
          </Link>
        );
